...
namespace CustomConcepts\TranssmartSmartConnect\Controller\Pickup;

use Bluebirdday\TranssmartSmartConnect\Model\Booking\ProfileRepository;
use Bluebirdday\TranssmartSmartConnect\Model\Carrier;
...
class Location extends \Bluebirdday\TranssmartSmartConnect\Controller\Pickup\Location
{
    /**
    ...
    public function execute()
    {
        ...
        // we don`t use search string
        //$searchString = $this->getRequest()->getParam('searchString');
        if ($this->getRequest()->getParam('checkAvailability')) {
            return $this->getResponse()->representJson(json_encode([
                'checkAvailability' => $this->currentProfile->locationsAvailabitity
            ]));
        }
    ...
    private function prepareLocationRequest() : array
    {
        ...
        return [
            ...
            'limit' => '50', // TODO: could be configured on backend
        ];
    }
