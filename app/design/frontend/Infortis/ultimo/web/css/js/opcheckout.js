...
Checkout.prototype = {
    ...
    // CUSTOM METHOD TO HANDLE THE BACKEND RESPONSE FROM THE FIRECHECKOUT
    updateLayoutBlocks: function(responseHtml, nextStep){
        var sections = responseHtml.update_section;
        Object.keys(sections).forEach(function(key){
            jQuery('.update-'+key).html(sections[key]);
        });
        this.gotoSection(nextStep, false);
    },
    ...
    setMethod: function(){
        ...
        document.body.fire('llogin:setMethod', {method : this.method});
    },
...
//        //this.accordion.openNextSection(true);
    ...
    update: function(url, params) {
        jQuery('#coupon-wait').fadeIn();
        var review = params.review;
        var formData = {'review': review};
        formData[params.name] = params.value;
        var request = new Ajax.Request(url, {
            method: 'post',
            onSuccess: this.handleFireCheckoutResponse.bind(this, review),
            onFailure: checkout.ajaxFailure.bind(checkout),
            parameters: formData
        });
    },

    // CUSTOM METHOD TO MAKE THE FIRECHECKOUT DISCOUNT CODE WORK
    handleFireCheckoutResponse: function(review, response){
        var parsedResponse = JSON.parse(response.responseText);
        if(parsedResponse){
            var sections = parsedResponse.update_section;
        }else{
            return false;
        }
        Object.keys(sections).forEach(function(key){
            // SORRY COULDN'T BE ARSED TO USE THE HORRIBLE PROTOTYPE
            jQuery('.update-'+key).html(sections[key]);
        })
        jQuery('#coupon-wait').fadeOut();
        if(review){
            jQuery('#coupon-code').hide();
            jQuery('#coupon-cancel').show()
        }else{
            jQuery('#coupon-code').show();
            jQuery('#coupon-cancel').hide()
        }
        setTimeout(function(){
            jQuery('#discount-code .messages').fadeOut();
        }, 5000);
    },
    ...
    setStepResponse: function(response){
        // **************************************************************************
        // UPDATING THE SECTIONS IS NOW HANDLED THROUGH THE UPDATELAYOUTBLOCKS METHOD
        // **************************************************************************
        ...
        // if (response.allow_sections) {
            ...
            // });
        // }
        // TODO: CHECK IF YOU NEED TO DO SOMETHING WITH THIS
...
Billing.prototype = {
    ...
    save: function(){
        ...
        if (validator.validate()) {
           ...
           //     $('billing:use_for_shipping_yes').value=1;
           // }
           //  console.log('hier kom ik wel toch?');
            new Ajax.Request(
                this.saveUrl,
                {
                    method: 'post',
    ...
    nextStep: function(transport){
        ...
        if(jQuery('#billing-address-select').length && jQuery('#billing-address-select').val() !== ""){
            fillSummaryFromSelect('billing')
        }else{
            fillSummary('billing');
        }
        checkout.updateLayoutBlocks(response, 'shipping_method');
...
//        $('billing:use_for_shipping_yes').checked = flag;
        ...
        if (!$('billing-address-select') || !$('billing-address-select').value) {
            ...
            for (var elemIndex in arrElements) {
                if (arrElements[elemIndex].id) {
                    ...
                    if (sourceField){
                        // TODO: CHECK FOR LOCALSTORAGE => ELSE USE THOSE
                        if(sourceField.value){
                            arrElements[elemIndex].value = sourceField.value;
                            // UPDATE FLOATING LABELS
                            arrElements[elemIndex].parentNode.classList.add('active')
                        }
                    }
                }
            }
    ...
    nextStep: function(transport){
        ...
        if(jQuery('#shipping-address-select').length && jQuery('#shipping-address-select').val() !== ""){
            fillSummaryFromSelect('shipping')
        }else{
            fillSummary('shipping');
        }
        checkout.updateLayoutBlocks(response, 'shipping_method');
        checkout.setStepResponse(response);

        // Custom handler to reset layout after setting shipping address
        if(jQuery('#shipformwrap.expanded').length){
            jQuery('#shipformwrap').removeClass('expanded');
            jQuery('#shipping_optionswrap').addClass('expanded')
        }
        /*
...
ShippingMethod.prototype = {
    ...
    nextStep: function(transport){
        ...
        checkout.updateLayoutBlocks(response, 'payment');
    }
};
